...
async def websocket_endpoint(websocket: WebSocket):
    """WebSocket endpoint. Expected messages (JSON):
    - {"action":"generate","text":"...","user":"reference"}
      -> server responds with {type: 'generated', character: {...}}
    - {"action":"play","text":"...","user":"reference"}
      -> server streams point events: {type:'point', stroke:int, x_norm, y_norm, timestamp}
    - {"action":"stop"}
    """
    ...
    try:
        while True:
            ...
            if action == 'generate':
                text = msg.get('text', '')
                user = msg.get('user', 'reference')
                if not text:
                    await websocket.send_text(json.dumps({'error': 'text required'}))
                    continue
                reset_loaded_user_splines()
                char = generate_sentence_character(text, user=user)
                points = character_to_points(char)
                await websocket.send_text(json.dumps({'type': 'generated', 'points': points}))
                ...
                reset_loaded_user_splines()
                char = generate_sentence_character(text, user=user)
                # Stream points in timestamp order; sleep according to timestamp deltas
                pts = []
                for stroke_count, stroke in enumerate(char.strokes):
                    for p in stroke:
                        pts.append({
                            'x_norm': p.x_norm,
                            'y_norm': p.y_norm,
                ...
                start_t = pts[0]['timestamp']
                prev_t = start_t
                for point in pts:
                    t = point['timestamp']
                    dt = max(0.0, t - prev_t)
                    # respect shutdown if client disconnected
                    await asyncio.sleep(dt)
                    point['timestamp'] = t - start_t  # normalize timestamp
                    await websocket.send_text(json.dumps({'type': 'point', **point}))
                    prev_t = t
                await websocket.send_text(json.dumps({'type': 'done'}))
